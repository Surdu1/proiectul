...
<nav class="navbar navbar-expand-lg navbar-light bg-light">
  <div class="container-fluid">
    ...
    <form class="d-flex mx-auto" action="search.php">
      ...
      <ul class="navbar-nav ml-auto mb-2 mb-lg-0">
        ...
        $sql = "SELECT * FROM melodi WHERE nume LIKE '%$input%' OR autor LIKE '%$input%' OR track LIKE '%$input%' OR pret LIKE '%$input%'";
        $result = mysqli_query($conectare,$sql);
        if(mysqli_num_rows($result)){
          echo '<div class="search_result">
          <div class="all">
          <div class="php_melodi_nume">
          <img  src="../icon/listen.svg">
          <h1>Muzica</h1>
          </div>
          </div>
              <div class="php_melodi">';
        $il = 0;
        while($row = $result -> fetch_assoc()){
          $il++;
          echo '
  ...
  <?php
    $category = false;
    $sql = "SELECT * FROM categori WHERE id LIKE '%$input%' OR nume LIKE '%$input%'";
...
if($melodie && $category){
  ...
  <div class="nfound">
   ...
   <h1>Ne pare rau, nu s-a gasit nici un rezultat</h1>
  </div>
  ';
}
?>
